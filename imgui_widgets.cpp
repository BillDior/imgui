...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data)
{
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    const bool is_align_to_middle = (flags & ImGuiInputTextFlags_AlignToMiddle) != 0;
    if (is_resizable)
    ...
    else
    {
        ...
        if (is_align_to_middle)
        {
            const ImVec2 sl_text_size = CalcTextSize(buf_display, NULL, true, 0);
            draw_pos.x = frame_bb.Max.x - frame_bb.GetSize().x * 0.5f - sl_text_size.x * 0.5f - style.FramePadding.x;
        }
